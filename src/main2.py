...
class login_UI(QtWidgets.QWidget):    
    def __init__(self):
        ...
        self.setWindowIcon(QIcon("C:/Users/gawoo/F/UI/logo.jpg"))  # Replace with the path to your icon file
        ...
        # Load the model and fix the reduction parameter issue
        model_path = 'C:/Users/gawoo/F/UI'
        self.model = load_model(os.path.join(model_path,'PostureDetection.h5'))
    ...
    def login_Def(self):
        try:
            ...
            # placeholders in the query prevent SQL injection
            ...
            if result:
                #self.close()
                username = result[0]  # username first column
                ...
                # Switch to the main page after a successful login
    ...
    def forgot_Def(self):
        try:
            ...
            if result:
                ...
                retrieved_username = result[0]  # username first column
                retrieved_password = result[1]  # password second column
    ...
    def onClicked(self):
        if self.capture_button_text == "START":
            self.checkInitialPosture()
            
        else:
            self.stopCapturing()

    def checkInitialPosture(self):
        cap = cv2.VideoCapture(0)
        ret, frame = cap.read()
        cap.release()

        if ret:
            # Save the captured frame to comp
            cv2.imwrite('C:/Users/gawoo/F/UI/captured_posture.jpg', frame)
            img = cv2.imread('C:/Users/gawoo/F/UI/captured_posture.jpg')
            ...
            if pred[0] > 0.5:
                self.showMessageBox2("Adjust Posture", "Please adjust your sitting posture according to the guidelines before starting!")
                self.stopCapture()
            else:
                self.showMessageBox2("Monitoring Started", "Your posture is good. Base Keypoints are all set. Monitoring has started.")
                self.startCapturing()
        else:
            self.showMessageBox("Error", "Failed to capture an image from the camera.")
    ...
    def ProfileClicked(self):
        try:
            ...
            if account_info:
                # update profile page with the retrieved information
